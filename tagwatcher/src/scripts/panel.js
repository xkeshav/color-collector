...
const Common = {
	createElement: (kind, classNames = [], idName = "") => {
		...
		if (classNames.length) elem.classList.add(...classList);
...
const render = (tag, count) => {
	...
	if (count > 0) {
		...
		const details = Common.createElement(
			"details",
			["details"],
			`details_${count}`
		);
...
chrome.devtools.network.onRequestFinished.addListener((event) => {
	...
	const contentHeader = response.headers.find(
